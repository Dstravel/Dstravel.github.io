...
## 제작을 하게 되면서 구현하고자 하는 것
 ...
 - 타이틀 화면
 시작하기,옵션,끝내기 버튼을 구현한다.
 옵션은 시간이 된다면 구현을 하고 안된다면 버튼만 구현한다.
 시작버튼은 저장화면으로 이동을
 끝내기버튼을 클릭하면 종료가 되게 구현한다.

 - 저장화면
 저장페이지는 총 3개를 구현한다.
 저장의 기능은 구현하지 않고 누르면 게임화면으로 바로 이동할 수 있도록만 구현한다.

게임화면
 - 일단 튜토리얼만 구현할수 있도록 한다.
 - 일반 사물은 불과 나무를 배치 나무는 충돌가능만 불은 충돌과 데미지를 구현한다.
 - 아이는 이동 및 스킬을 구현한다.
 - 남자는 자동공격을 구현(가능하면 스킬도)
 - 적오브젝트는 공격패턴 및 플레이어 캐릭터 공격을 구현
...
## 결과
 ...
 시작화면 및 저장페이지 화면 설계. 설정화면은 아직 못함(75%)
  ...
  - 게임에 사용항 오브젝트=> 남자에 사용할 괜찮은 캐릭은 찾았으나 아이와 적 오브젝트는 아직 찾지 못함=>20%
  - 사물중에 바리케이트는 찾지못해 나무로 대체, 불 오브젝트는 아직 찾는 중=> 50%
  
  - 배치설계는 완료. 하지만 제작중에는 변경될수 있음=> 100%
 ...
 2)시작화면에는 게임시작, 설정, 끝내기버튼, 총 3가지가 있다.
  =>설정화면에 들어갈 요소들을 설계해서 구현하기

 3)끝내기버튼 클릭시 화면이 종료된다.

 4)기본적으로 맵은 가로로 긴 직사각형 모양의 맵이다.

 5)시작하면 몇분 가량의 튜토리얼을 하게된다.
 (일단은 튜토리얼 부분은 완성 시킬수 있도록한다)
 ...
 4)보스몬스터는 일반 몬스터와 비슷하지만 좀더 화려하고 덩치가 큰걸로 찾기
 =>(만약 힘들경우 일반 몬스터 덩치를 키워서 사용하기) 

 5)아직 구하지 못한 일반 몬스터 에셋을 찾아서 오브젝트와 함께 배치하기(튜토리얼 파트부분만)
 =>현재 몬스터 1개의 에셋을 찾지 못했음)
...
변경해서 남자 에셋(혹은 비슷한)을 찾아서 작업
=>비슷한 에셋을 찾아서 작업함(100%)

오브젝트 물리충돌외에 프로그래밍 작업
=>물리충돌작업 프로그래밍은 끝났으나 문제가 없는지 체크를 하지 못함(90%)

NPC(남자) AI와 적 AI 설계하고 프로그래밍해서 프로토타입 만들기
=>프로토 타입 제작은 완성했으나 중간중간 문제가 있어 스크립트 수정이 필요함(60%) 
...
중간중간 발생하는 버그 수정하기 
맵 오브젝트중 하나인 불에 데미지주는 프로그래밍하기
몬스터 배치 및 전투 구현 하기(3기정도)
...
4. 게임화면에 캐릭터와 NPC배치는 하였으나 프로그래밍 오류 및 완성을 하지 못해서 완료를 다 하지 못한 상태이다(6주차에 마무리 할 예정).=> 50%
...
(애니메이션 오류 및 버그 등등의 문제가 아직 남아있음)
 ...
 3. 영상에서 봤듯이 오류난거 수정하고 전투에 필요한 목표지정 시스템 구현하기=>두번째 우선순위  
